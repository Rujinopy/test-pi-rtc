/*
 * Conductor Class Implementation
 * 
 * This class serves as the main controller for WebRTC functionality, handling:
 * - Peer connection management
 * - Audio/Video track initialization
 * - Media capture (camera and audio)
...
/**
 * Creates a new Conductor instance with the specified configuration
 * @param args Configuration parameters for the WebRTC session
 * @return Shared pointer to the configured Conductor instance
 */
...
/**
 * Initializes media tracks for both audio and video
 * - Creates audio track if audio is enabled
 * - Creates video track if camera is specified
 * - Supports both libcamera and V4L2 capture methods
 */

void Conductor::InitializeTracks() {
    //Audio Track Initilization
...
/**
 * Adds configured audio and video tracks to a peer connection
 * @param peer_connection The peer connection to add tracks to
 * - Sets up stream IDs
 * - Configures video quality parameters
 * - Maintains framerate over resolution when needed
 */
...
/**
 * Creates a new WebRTC peer connection with specified configuration
 * @param peer_config Configuration for the peer connection
 * @return Scoped reference to the created RtcPeer
 * - Sets up STUN/TURN servers
...
/**
 * Handles snapshot requests from peers
 * @param datachannel Channel to send snapshot data
 * @param msg Message containing quality parameters
 * - Captures current frame
 * - Converts to JPEG format
 * - Sends via data channel
 */
...
/**
 * Event handlers for metadata operations
 * - LATEST: Retrieves most recent recording
 * - OLDER: Retrieves historical recordings
 * - SPECIFIC_TIME: Retrieves recording from specific timestamp
 */
 ...
 * Allows remote adjustment of camera parameters
 * @param datachannel Channel for camera control commands
 * @param msg JSON message containing control parameters
 */
...
void Conductor::InitializePeerConnectionFactory() {
    // Initialize SSL for secure WebRTC connections
    rtc::InitializeSSL();
    
    // Create three main WebRTC threads:
    // 1. Network thread: handles all network I/O
    // 2. Worker thread: handles media processing and encoding/decoding
    // 3. Signaling thread: handles signaling protocol operations
    network_thread_ = rtc::Thread::CreateWithSocketServer();
    worker_thread_ = rtc::Thread::Create();
    signaling_thread_ = rtc::Thread::Create();

    // Start all threads and log success messages
    ...
    // Set up main factory dependencies structure
    webrtc::PeerConnectionFactoryDependencies dependencies;
    // Assign the three main threads
    dependencies.network_thread = network_thread_.get();
    dependencies.worker_thread = worker_thread_.get();
    dependencies.signaling_thread = signaling_thread_.get();
    
    // Create supporting components:
    // - Task queue factory: manages async operations
    // - Call factory: handles WebRTC call setup
    // - Event logging: for debugging and monitoring
    // - Field trials: for A/B testing features
    ...
    // Set up media engine dependencies for audio/video handling
    cricket::MediaEngineDependencies media_dependencies;
    media_dependencies.task_queue_factory = dependencies.task_queue_factory.get();

    // Configure audio device module
    // Default to PulseAudio on Linux, fallback to dummy if audio disabled
    ...
    // - Audio device module: handles audio I/O
    // - Audio encoders/decoders: for audio compression
    // - Audio processing: handles echo cancellation, noise suppression, etc.
    ...
    media_dependencies.audio_mixer = nullptr;  // Use default audio mixing

    // Set up video components:
    // - Custom video encoder factory: for hardware acceleration support
    // - Video decoder factory: supports multiple codecs (H264, VP8, VP9, AV1)
    ...
    // Link trials configuration
    media_dependencies.trials = dependencies.trials.get();
    
    // Create the media engine with all configured components
    dependencies.media_engine = cricket::CreateMediaEngine(std::move(media_dependencies));

    // Finally, create the peer connection factory with all dependencies
